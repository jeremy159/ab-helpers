    ...
    if args.dry_run {
        ...
        return match service.run::<DryRun>().await {
            Ok(DryRunOutcome::Skip(InterestSkip::AccountClosed)) => {
                tracing::info!("mortgage account is closed - skipping - (DRY-RUN)");
                Ok(ExitCode::SUCCESS)
            }
            Ok(DryRunOutcome::Skip(InterestSkip::NoInterest { balance, cutoff })) => {
                tracing::info!(balance, %cutoff, "no mortgage interest to apply - (DRY-RUN)");
                Ok(ExitCode::SUCCESS)
            }
